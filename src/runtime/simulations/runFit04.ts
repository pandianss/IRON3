...
import { SIM_FIT_04 } from './src/simulation/scenarios/SIM-FIT-04.js';
import { MetricCollector } from './src/simulation/harness/MetricCollector.js';
import fs from 'fs';
import path from 'path';

async function run() {
    console.log(`--- IRON INSTITUTIONAL SCIENCE: ${SIM_FIT_04.name} ---`);
    const sim = new SimulationKernel(SIM_FIT_04);
    ...
    const outputDir = path.join(process.cwd(), 'outputs', 'SIM-FIT-04');
    ...
    const archetypes = Object.keys(SIM_FIT_04.archetypes);
